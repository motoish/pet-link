...
Oxfmt while preserving the current correctness and architecture rules, TypeScript checks,
...
the single owner of whitespace and import ordering. The migration intentionally
drops `simple-import-sort/exports` because Oxfmt does not provide an equivalent
export-declaration sorting rule, and retaining an ESLint plugin for that style
rule would defeat the dependency simplification goal.
